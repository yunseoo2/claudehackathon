    ...
    <div className="bg-white dark:bg-dark-700/30 border border-border dark:border-dark-600 rounded-xl p-8 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {documents.map((doc) => (
          <div
            key={doc.id}
            className="p-6 border border-gray-200 dark:border-dark-600 rounded-lg bg-gray-50/50 dark:bg-dark-700/50 hover:bg-gray-100/50 dark:hover:bg-dark-600/50 hover:border-gray-300 dark:hover:border-dark-500 transition-all"
          >
            <div className="flex flex-col h-full">
              <div className="flex items-center gap-2 mb-3">
                <h3 className="font-semibold text-gray-900 dark:text-white flex-1">{doc.title}</h3>
                {doc.critical && (
                  <span className="text-xs bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 px-2.5 py-1 rounded-full font-medium">
                    Critical
                  </span>
                )}
              </div>
              ...
              <div className="space-y-2 flex-1">
                ...
                {isStale(doc.lastUpdated) && (
                  <div className="flex items-center gap-2 text-sm text-yellow-700 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 px-3 py-2 rounded-lg border border-yellow-200 dark:border-yellow-900/50">
                    <span>⚠</span>
